...
class Security(Mobile):
    ...
    def _recv_process(self, _wsmData):
        ...
        if _msgType == "signedData":
            _hashId = self.get_hashId()
            print("\tGet HashId: {}".format(_hashId))

            print("\nParsing tbsData")
            print("\tGet Payload: {}".format(_msgData))

            _psid = self.get_psid()
            print("\nParsing HeaderInfo")
            print("\tGet PSID: {}".format(_psid))

            _genTime = self.get_generationtime()

            if self._checkGenerateTime(_genTime):
                print("\tGet Generation Time: {}".format(_genTime))

                _sigType, _sigData = self.get_signer()
                print("\nParsing Signer")
                print("\tGet Signer Type: {}".format(_sigType))

                if _sigType == "certificate":
                    print("\tGet Version: {}".format(self.get_certVersion()))
                    print("\tGet Issuer: {}".format(self.get_issuer()))
                    # print("\tGet Certificate: {}".format(self.get_certificate()))

                    _type, _val = self.get_id()
                    print("\nParsing toBeSigned")

                    if _type == "linkageData":
                        _iCert = self.get_iCert()
                    ...
                    _status, _appPermission = self.get_appPermission()
                        ...
                        def selectLane():
                            if self._changeLane:
                                if len(self._enableLane) != 0:
                                    ...
                                    print("\nSelecting Next Lane")
                                    print("\tGet previous Lane: {}".format(self._lane))

                                    self._nextLane = self._enableLane[0]
                                    print("\tGet Available Lanes: {}".format(self._enableLane))
                                    print("\tSelected Next Lane: {}".format(self._nextLane))

                                    self._changeLane = False
                                    self._enableLane.clear()
                            else:
                                print("\nCar is Moving. Current Lane: {}".format(self._lane))

                        if _status: break  # 신호등이 두 개라서 상태가 있으면 다음 걸 볼 필요가 없

                        if self._lane == 5 and _signalGroup == 1:
                            if _eventState == "protected-Movement-Allowed":
                                selectLane()
                                _status = "\nCar can go. The next lane is {}!".format(self._lane)

                            elif _eventState == "stop-And-Remain":
                                selectLane()
                                _status = "\nCar can't go now. The next lane is {}!".format(self._lane)

                        elif self._lane == 10 and _signalGroup == 2:
                            if _eventState == "protected-Movement-Allowed":
                                selectLane()
                                _status = "\nCar can go. The next lane is {}!".format(self._lane)
                            ...
                            selectLane()
                            _status = "\nCar can go now. The next lane is {}!".format(self._lane)
                    else:
    ...
    def _receive_map(self, _msgData):
        ...
        for _cnt in range(_laneSetLen):
            ...
            for _cnt2 in range(_connectsTo):
                ...
                if _laneID == self._lane and _connectLane not in self._enableLane:
    ...
    def _generate_bsm(self):
        ...
        bsm.set_speed(self._speed)  # parsing
    ...
    def _generate_eva(self):
        ...
        self._psid = 0x82
        attack_cert = pickle.load(open("attacker/scenario1.pkl", "rb"))
        attack_key = ECC.import_key(open("attacker/scenario2_key.pem", "rt").read())
        dot2data = self._createIeee1609Dot2Data(_dot2Data=eva, _signed=True, certificate=attack_cert, key=attack_key)
        dot3data = self._createIeee1609Dot3Data(_dot3Data=dot2data)
        return dot3data
    ...
    def _generate_srm(self):
        ...
        srm.set_inBoundLane(IntersectionAccessPoint(_lane=self._lane))  # 원하는 진입 접근 방식 또는 차선
        ...
        srm.set_pos_speed(self._speed)  # parsing (optional)
        srm = srm.createSRM()
