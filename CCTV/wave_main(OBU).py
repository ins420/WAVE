...
class EE(Security):
    ...
    def start(self):
        ...
        # TODO Scenario 3
        # send_thread = threading.Thread(target=self._send_wave, kwargs={"_pkt": self._generate_eva})
        # send_thread.start()

        lane = threading.Thread(target=self.changeLanes)
        lane.start()

        recv_thread = threading.Thread(target=self._recv_wave)
        recv_thread.start()

    def changeLanes(self):   # OBU
        start = time.time() + 2
        while True:
