...
class EE(Security):
    ...
    def start(self):
        ...
        send_thread = threading.Thread(target=self._send_wave, kwargs={"_pkt": self._generate_bsm, "_inter": 1})
        send_thread.start()
    ...
    def _recv_wave(self):
        while True:
            sniff(iface=fixed["interface"], stop_filter=self._recv_data, store=0)
