...
class ASN(Ieee1609Dot3):
    ...
    def _certHash(self):
        for path in glob("cert_file/*pkl"):
            certificate = pickle.load(open(path, "rb"))
            encoded = dot2encoder.encode("Certificate", certificate)
            hashedid8 = SHA256.new(encoded).hexdigest()[-8:]
            self._certLst[hashedid8] = "key_file/{}_key.pem".format(path.split('/')[1].split('.')[0])

        print("\nStored Cert List\n{}".format(self._certLst))
