...
certificate = pickle.load(open("cert_file/application_cert/rsu1.pkl", "rb"))
...
class Ieee1609Dot2:
    ...
    def _signedData(self):
        ...
        _content.set_value("signer", ("certificate", [certificate]))
    ...
    def get_signer(self):
        _content = self._dot2data["content"][1]
        _signer = _content["signer"]
        return _signer[0], _signer[1][0]

    def get_certificate(self):
        _content = self._dot2data["content"][1]
        _certificate = _content["signer"][1][0]
        return _certificate

    def get_psid(self):
        _psid = self._dot2data["tbsData"]["headerInfo"]["psid"]
        return _psid

    def get_generationtime(self):
        _generation = self._dot2data["tbsData"]["headerInfo"]["generationTime"]
        return _generation

    def get_rSig(self):
        _content = self._dot2data["content"][1]
        _rSig = _content["signature"][1]["rSig"][1]
        return self.printObj(_rSig)

    def get_sSig(self):
        _content = self._dot2data["content"][1]
        _sSig = _content["signature"][1]["sSig"]
        return self.printObj(_sSig)

    def get_certVersion(self):
        _version = self._dot2data["signer"][1][0]["version"]
        return _version

    def get_issuer(self):
        _issuer = self.printObj(self._dot2data["signer"][1][0]["issuer"][1])
        return _issuer

    def get_tbsdata(self):
        _content = self._dot2data["content"][1]
        return _content["tbsData"]

    def get_id(self):
        _type, _val = self._dot2data["signer"][1][0]["toBeSigned"]["id"]
        return _type, _val

    def get_iCert(self):
        _iCert = self._dot2data["signer"][1][0]["toBeSigned"]["id"][1]["iCert"]
        return _iCert

    def get_linakgeValue(self):
        _linkage = self.printObj(self._dot2data["signer"][1][0]["toBeSigned"]["id"][1]["linkage-value"])
        return _linkage

    def get_cracaId(self):
        _cracaId = self.printObj(self._dot2data["signer"][1][0]["toBeSigned"]["cracaId"])
        return _cracaId

    def get_crlSeries(self):
        _crlSeries = self._dot2data["signer"][1][0]["toBeSigned"]["crlSeries"]
        return _crlSeries

    def get_region(self):
        _type, _val = self._dot2data["signer"][1][0]["toBeSigned"]["region"][1][0]
        return _type, _val

    def get_appPermission(self):
        _status = ("appPermissions" in self._dot2data["signer"][1][0]["toBeSigned"]) is True
        if _status: return _status, self._dot2data["signer"][1][0]["toBeSigned"]["appPermissions"]
        else: return _status, None

    def get_valStart(self):
        _valStart = self._dot2data["signer"][1][0]["toBeSigned"]["validityPeriod"]["start"]
        return _valStart

    def get_valDuration(self):
        _type, _val = self._dot2data["signer"][1][0]["toBeSigned"]["validityPeriod"]["duration"]
        return _type, _val

    def get_verifyIndicator(self):
        _indicator = self._dot2data["signer"][1][0]["toBeSigned"]["verifyKeyIndicator"]
        if _indicator[0] == "verificationKey":
            _type, _val = self._dot2data["signer"][1][0]["toBeSigned"]["verifyKeyIndicator"][1][1]
            return _type, self.printObj(_val)
        elif _indicator[0] == "reconstructionValue":
            _type, _val = self._dot2data["signer"][1][0]["toBeSigned"]["verifyKeyIndicator"][1]
            return _type, self.printObj(_val)
