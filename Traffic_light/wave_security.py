...
bar2_pwm = 12
signalGroup1_red = 4
signalGroup1_green = 3
signalGroup2_red = 21
signalGroup2_green = 5

fixed = {
    "interface": "wlan1",
    "addr": "bb:bb:bb:bb:bb:42",
...
class Security(Mobile):
    def __init__(self):
        super().__init__()
        # self._lane_ = 1  # OBU
        # self._nextLane_ = None
        # self._changeLane_ = False
        self._width = 23  # OBU
        self._length = 27  # OBU
    ...
    def signalGroup_state(self):
        GPIO.cleanup()
        GPIO.setmode(GPIO.BCM)
        GPIO.setup(signalGroup1_red, GPIO.OUT)
        GPIO.setup(signalGroup1_green, GPIO.OUT)
        GPIO.setup(signalGroup2_red, GPIO.OUT)
        GPIO.setup(signalGroup2_green, GPIO.OUT)
        GPIO.setup(bar1_pwm, GPIO.OUT)  #
        GPIO.setup(bar2_pwm, GPIO.OUT)  #
        bar1 = GPIO.PWM(bar1_pwm, 50)
        bar2 = GPIO.PWM(bar2_pwm, 50)
        ...
        while True:
            if self._inter2State == "stop-And-Remain":
                GPIO.output(signalGroup1_red, GPIO.HIGH)
                GPIO.output(signalGroup1_green, GPIO.LOW)
                GPIO.output(signalGroup2_red, GPIO.LOW)
                GPIO.output(signalGroup2_green, GPIO.HIGH)
                bar1.ChangeDutyCycle(3)
                bar2.ChangeDutyCycle(10)

            elif self._inter1State == "stop-And-Remain":
                GPIO.output(signalGroup1_red, GPIO.LOW)
                GPIO.output(signalGroup1_green, GPIO.HIGH)
                GPIO.output(signalGroup2_red, GPIO.HIGH)
                GPIO.output(signalGroup2_green, GPIO.LOW)
                bar1.ChangeDutyCycle(7.5)
                bar2.ChangeDutyCycle(5)
    ...
    def _receive_bsm(self, _msgData):
        ...
        if _msgObj._partII:
            if _msgObj._partId == 0x00:
                ...
                print("\nThe Car detected disabled vehicle!")
    ...
    def _receive_rsa(self, _msgData, _eva=False):
        ...
        if self._checkTimestamp(_timeStamp):
            ...
            print("\nAmbulance is coming!") if _eva else print("\nObstacle is detected!")
            return True
        else:
            print("\nThe timestamp is invalid")
                        ...
                        if _status == "requested":
                            ...
                            if _inBoundLane == 10:
                                if self._inter2State == "stop-And-Remain":
                                    ...
                                    if self._sigTime - self._minEndTime > 200:
                                        self._sigTime = 170
                                else:
                                    self._sigTime += 100
    ...
    def _receive_map(self, _msgData):
        ...
        if True:
            ...
            self.selectLane()
            #print("Enable Lane : {}".format(self._enableLane))
            #print("\tProcessed Time : {} ".format(int(time.time()-start)))
            print("\tLane : {} , Next : {}, Enable : {}".format(self._lane_, self._nextLane_, self._enableLane))
        return True
    ...
    def _generate_spat(self):  # b.pcap (in 2022졸업작품>WAVE>WAVE패킷) / no.4
        ...
        self._psid = 0x82
        dot2data = self._createIeee1609Dot2Data(_dot2Data=spat, _signed=True)
        dot3data = self._createIeee1609Dot3Data(_dot3Data=dot2data)
        return dot3data
    ...
    def _generate_rsa(self, _eva=False):
        ...
        self._psid = 0x82
        dot2data = self._createIeee1609Dot2Data(_dot2Data=rsa, _signed=True)
        dot3data = self._createIeee1609Dot3Data(_dot3Data=dot2data)
        return dot3data
    ...
    def _generate_eva(self):
        ...
        self._psid = 0x82
        dot2data = self._createIeee1609Dot2Data(_dot2Data=eva, _signed=True)
        dot3data = self._createIeee1609Dot3Data(_dot3Data=dot2data)
        return dot3data
    ...
    def _generate_srm(self):
        ...
        self._psid = 0x82
        dot2data = self._createIeee1609Dot2Data(_dot2Data=srm, _signed=True)
        dot3data = self._createIeee1609Dot3Data(_dot3Data=dot2data)
        return dot3data
    ...
    def _generate_ssm(self, _reqInfo: dict):
        ...
        self._psid = 0x82
        dot2data = self._createIeee1609Dot2Data(_dot2Data=ssm, _signed=True)
        dot3data = self._createIeee1609Dot3Data(_dot3Data=dot2data)
        return dot3data
    ...
    def _generate_map(self):
        ...
        self._psid = 0x82
        dot2data = self._createIeee1609Dot2Data(_dot2Data=mapData, _signed=True)
        dot3data = self._createIeee1609Dot3Data(_dot3Data=dot2data)
        return dot3data
